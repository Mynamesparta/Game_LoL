...
void RSA::Generation()
{
    ...
    bool* is_norm_inverse=new bool(0);
    while(!*is_norm_inverse)
    {
        _public_key_e=norm_Random(2,e_f-1);
        _private_key_d=Modular_Multiplicative_Inverse(_public_key_e,e_f,is_norm_inverse);
        if(_public_key_e==_private_key_d)
        {
            (*is_norm_inverse)=false;
            continue;
        }
    }
    ...
    qDebug()<<qR<<"test: public * private % mod"<<(_public_key_e*_private_key_d)%e_f;
}

LongInt& RSA::public_key_mod()
{
    return _public_key_mod;
}

LongInt& RSA::public_key_e()
{
    return _public_key_e;
}

LongInt& RSA::private_key_d()
{
    return _private_key_d;
}

void RSA::setPublicKey(LongInt e,LongInt mod)
{
    _public_key_e=e;
    _public_key_e=mod;
    qDebug()<<qR<<"new public key ("<<_public_key_e<<","<<_public_key_mod<<")";
}

void RSA::setPrivateKey(LongInt d)
{
    _private_key_d=d;
    qDebug()<<qR<<"new private key ("<<_private_key_d<<")";
}
